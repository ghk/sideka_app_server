...
from tatakelola_kabupaten.models import SdContent
...
class SidekaContentRepository(BaseRepository):
    ...
    def get_latest_content(self, type, subtype, desa_ids=None):
        ...
        if (desa_ids):
            subq = subq.filter(self.model.desa_id.in_(desa_ids))

        subq = subq.group_by(self.model.desa_id).subquery()
