...
def post_content_v2(desa_id, content_type, key, content_subtype=None):
    ...
    try:
		...
		if content_subtype != 'subtype':
			...
			cur.execute("INSERT INTO sd_contents(desa_id, type, subtype, content, date_created, created_by, change_id, api_version) VALUES(%s, %s, %s, %s, now(), %s, %s)", (desa_id, content_type, content_subtype, json_new_content, user_id, new_change_id, '2.0'))
			mysql.connection.commit()
...
def post_map_content(desa_id, change_id = None):
	 ...
	 try:
		...
		json_new_content = json.dumps(new_content)
		cur.execute("INSERT INTO sd_contents(desa_id, type, subtype, content, date_created, created_by, change_id, api_version) VALUES(%s, %s, %s, %s, now(), %s, %s)", (desa_id, 'map', None, json_new_content, user_id, new_change_id, '2.0'))
		mysql.connection.commit()
		suceess = True
